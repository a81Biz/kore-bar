...
describe('Módulo KDS — Routing explícito por categoría (route_to_kds)', () => {
    ...
    const CAT_COCINA = 'KDSR-CAT-FOOD';
    const CAT_BEBIDAS = 'KDSR-CAT-DRINK';
    const DISH_TACO = 'KDSR-TACO-01';
    const DISH_AGUA = 'KDSR-AGUA-01';
    const DISH_MARGARITA = 'KDSR-MARGARITA-01';
    ...
    beforeAll(async () => {
        ...
        // PIN via API — bcrypt hash, compatible con auth.helper.js
        await app.request(`/api/admin/employees/${TEST_EMP}/reset-pin`, {
            method: 'PUT',
        ...
        // Categoría BEBIDAS (route_to_kds = FALSE)
        await app.request('/api/admin/menu/categories', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        // Platillo TACO — categoría cocina, con receta
        await app.request('/api/admin/menu/dishes', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ dishCode: DISH_TACO, categoryCode: CAT_COCINA, name: 'Taco KDSR', price: 90 })
        });
        ...
        await app.request('/api/kitchen/recipes/items', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ dishCode: DISH_TACO, ingredientCode: 'KDSR-ING-TORTILLA', quantity: 3 })
        });
        ...
        await app.request('/api/admin/menu/dishes', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ dishCode: DISH_AGUA, categoryCode: CAT_BEBIDAS, name: 'Agua Mineral KDSR', price: 35 })
        });
        ...
        await app.request('/api/admin/menu/dishes', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ dishCode: DISH_MARGARITA, categoryCode: CAT_BEBIDAS, name: 'Margarita KDSR', price: 95 })
        });
        await app.request('/api/kitchen/ingredients', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        await app.request('/api/kitchen/recipes/items', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ dishCode: DISH_MARGARITA, ingredientCode: 'KDSR-ING-TEQUILA', quantity: 60 })
        });
    });
    ...
    afterAll(async () => {
        await executeQuery(null, `
            DELETE FROM order_items WHERE order_id IN (
                SELECT id FROM order_headers WHERE table_id = (SELECT id FROM restaurant_tables WHERE code = $1)
            )`, [TEST_TABLE]);
        await executeQuery(null, `
            DELETE FROM order_headers WHERE table_id = (SELECT id FROM restaurant_tables WHERE code = $1)
        `, [TEST_TABLE]);
        await executeQuery(null, `
            DELETE FROM dish_recipes WHERE dish_id IN (SELECT id FROM menu_dishes WHERE code IN ($1, $2))
        `, [DISH_TACO, DISH_MARGARITA]);
        await executeQuery(null, `DELETE FROM menu_dishes WHERE code IN ($1, $2, $3)`, [DISH_TACO, DISH_AGUA, DISH_MARGARITA]);
        await executeQuery(null, `DELETE FROM menu_categories WHERE code IN ($1, $2)`, [CAT_COCINA, CAT_BEBIDAS]);
        await executeQuery(null, `DELETE FROM inventory_items WHERE code IN ('KDSR-ING-TORTILLA', 'KDSR-ING-TEQUILA')`);
    ...
    const sendDish = async (dishCode) => {
        ...
        await app.request(`/api/waiters/tables/${TEST_TABLE}/orders`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ employeeNumber: TEST_EMP, items: [{ dishCode, quantity: 1 }] })
        });
    };
    ...
    it('KDSR-01 La columna route_to_kds existe en menu_categories con tipo boolean', async () => {
        const check = await executeQuery(null, `
            ...
            WHERE table_name = 'menu_categories' AND column_name = 'route_to_kds'
        `);
        expect(check.length).toBe(1);
        expect(check[0].data_type).toBe('boolean');
    });

    it('KDSR-02 El default de route_to_kds es TRUE', async () => {
        const check = await executeQuery(null, `
    ...
    it('KDSR-03 La categoría de cocina tiene route_to_kds=true', async () => {
        const check = await executeQuery(null, `SELECT route_to_kds FROM menu_categories WHERE code = $1`, [CAT_COCINA]);
        expect(check[0].route_to_kds).toBe(true);
    });

    it('KDSR-04 La categoría de bebidas tiene route_to_kds=false', async () => {
        const check = await executeQuery(null, `SELECT route_to_kds FROM menu_categories WHERE code = $1`, [CAT_BEBIDAS]);
        expect(check[0].route_to_kds).toBe(false);
    });

    // ═══════════════════════════════════════════════════════════
    // Routing al enviar comanda
    // ═══════════════════════════════════════════════════════════

    it('KDSR-05 Platillo con receta y route_to_kds=true → PENDING_KITCHEN', async () => {
        await sendDish(DISH_TACO);
        expect(await getLastItemStatus(DISH_TACO)).toBe('PENDING_KITCHEN');
    });

    it('KDSR-06 Bebida sin receta y route_to_kds=false → PENDING_FLOOR', async () => {
        await sendDish(DISH_AGUA);
        expect(await getLastItemStatus(DISH_AGUA)).toBe('PENDING_FLOOR');
    });

    it('KDSR-07 Coctel CON receta pero route_to_kds=false → PENDING_FLOOR', async () => {
        await sendDish(DISH_MARGARITA);
        expect(await getLastItemStatus(DISH_MARGARITA)).toBe('PENDING_FLOOR');
    });
    ...
    it('KDSR-08 El tablero de cocina no incluye items de bebidas (PENDING_FLOOR)', async () => {
        ...
        const allItems = [...(board.pending || []), ...(board.preparing || []), ...(board.ready || [])];
        const drinkInKds = allItems.find(i => i.dishName === 'Agua Mineral KDSR' || i.dishName === 'Margarita KDSR');
    ...
    it('KDSR-09 El tablero de cocina SÍ incluye el taco (PENDING_KITCHEN)', async () => {
        ...
        const board = json.data.board;
        const allItems = [...(board.pending || []), ...(board.preparing || []), ...(board.ready || [])];
        expect(allItems.find(i => i.dishName === 'Taco KDSR')).toBeDefined();
    });

    it('KDSR-10 Una comanda mixta enruta cada item correctamente', async () => {
        await executeQuery(null, `
            UPDATE order_headers SET status = 'CLOSED'
            WHERE table_id = (SELECT id FROM restaurant_tables WHERE code = $1)
              AND status IN ('OPEN', 'AWAITING_PAYMENT')
        ...
        await app.request(`/api/waiters/tables/${TEST_TABLE}/orders`, {
            ...
            body: JSON.stringify({
                ...
                items: [
                    { dishCode: DISH_TACO, quantity: 1 },
                    { dishCode: DISH_AGUA, quantity: 1 },
                    { dishCode: DISH_MARGARITA, quantity: 1 }
                ]
            })
        });

        const items = await executeQuery(null, `
            SELECT md.code AS dish_code, oi.status FROM order_items oi
            JOIN order_headers oh ON oh.id = oi.order_id
            JOIN restaurant_tables rt ON rt.id = oh.table_id
            JOIN menu_dishes md ON md.id = oi.dish_id
            WHERE rt.code = $1 AND oh.status = 'OPEN' AND md.code IN ($2, $3, $4)
